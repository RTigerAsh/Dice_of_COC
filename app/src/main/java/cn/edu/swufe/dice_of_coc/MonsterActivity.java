...
public class MonsterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //获取SP里的页数数据
        SharedPreferences sharedPreferences=getSharedPreferences("monsterpage", Activity.MODE_PRIVATE);
        final int myPage = (int)sharedPreferences.getInt("monsterpageINT",0);

        Log.i("MonsterActivity  sp", "数据以保存到sharedPreferences");

        //final int myPage = (int) SPUtils.get("page", 0);
        //选择pdf
        pdfView.fromAsset("monster.pdf")
...
//                .onRender(onRenderListener)
    ...
    private void getPermission() {
        int hasWriteContactsPermission = ContextCompat.checkSelfPermission(MonsterActivity.this,
                Manifest.permission.READ_EXTERNAL_STORAGE);
        Log.i("getPermission", "getPermission: 运行方法");
        if (hasWriteContactsPermission != PackageManager.PERMISSION_GRANTED) {
            if (!ActivityCompat.shouldShowRequestPermissionRationale(MonsterActivity.this,
                    Manifest.permission.READ_EXTERNAL_STORAGE)) {
                ActivityCompat.requestPermissions(MonsterActivity.this,
        ...
        while ((ContextCompat.checkSelfPermission(MonsterActivity.this,
    ...
    protected void onDestroy() {
        ...
        editor.putInt("monsterpageINT",p);
        editor.commit();
